...
public class MemberGUI extends JPanel implements ActionListener, MouseListener, KeyListener, FocusListener {
	JButton[] buttons;
	String[] btnTitle = { "추가", "수정", "탈퇴", "취소", "아이디중복" };
	...
	Object[] columnNames = { "아이디", "이름", "Email", "연락처", "생년월일", "주소", "성별", "가입일", "권한", "비밀번호" };
	...
	private void setTableData(List<MemberDTO> list) {
	...
	private void addListener() {
		...
		tfSearch.addKeyListener(this); // 키이벤트
		tfSearch.addFocusListener(this); // 키이벤트 텍스트안에 포커스가 들어가 있음을 구분하기 위함
	...
	public boolean checkInput(int flag) {
		...
		if (flag == ALL) {
			if (tfId.getText().trim().equals("")) {
				JOptionPane.showMessageDialog(MemberGUI.this, "아이디를 입력하세요!");
				tfId.requestFocus();
				return false;
			} else if (buttons[4].isEnabled()) {
				JOptionPane.showMessageDialog(MemberGUI.this, "아이디 중복확인해주세요");
				tfId.requestFocus();
				return false;
			} else if (!String.valueOf(tfPwd1.getPassword()).equals(String.valueOf(tfPwd2.getPassword()))) {
				JOptionPane.showMessageDialog(MemberGUI.this, "비밀번호가 일치하지 않습니다!");
				tfPwd2.requestFocus();
				return false;
			} else if (tfId.getText().trim().length() > 16) {
				JOptionPane.showMessageDialog(MemberGUI.this, "아이디는 15자리 이하로 입력하세요!");
				tfId.requestFocus();
				return false;
			} else if (String.valueOf(tfPwd1.getPassword()).trim().length() == 0) {
				JOptionPane.showMessageDialog(MemberGUI.this, "비밀번호를 입력하세요!");
				tfPwd1.requestFocus();
				return false;
			} else if (String.valueOf(tfPwd1.getPassword()).trim().length() > 16) {
				JOptionPane.showMessageDialog(MemberGUI.this, "비밀번호는 15자리 이하로 입력하세요!");
				tfPwd1.requestFocus();
				return false;
			} else if (tfName.getText().trim().length() == 0) {
				JOptionPane.showMessageDialog(MemberGUI.this, "이름을 입력하세요!");
				tfName.requestFocus();
				return false;
			} else if (tfHp.getText().trim().length() == 0) {
				JOptionPane.showMessageDialog(MemberGUI.this, "전화번호를 확인하세요!");
				tfName.requestFocus();
				return false;
			} else if (!male.isSelected()&&!feMale.isSelected() ) {
				JOptionPane.showMessageDialog(MemberGUI.this, "성별체크를 확인하세요!");
				tfName.requestFocus();
				return false;
			}
		}
			...
			if ((tfId.getText().trim().equals("") && tfName.getText().trim().length() == 0)
				...
				JOptionPane.showMessageDialog(MemberGUI.this, "유효한 검색 문자열이 아니예요!");
				return false;
			}
		}
		return true;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == buttons[0]) { // 추가
			...
			int rs = dao.insert(id, pwd1, pwd2, name, email, hp, addr, gender, master, birth);

			if (rs == 1) {
				JOptionPane.showMessageDialog(buttons[0], "입력 성공했어요!");
			} else
				JOptionPane.showMessageDialog(buttons[0], "입력 실패했어요!");
		...
		if (obj == buttons[4]) {// 아이디체크
			...
			if (tfId.getText().trim().equals("")) {
				...
				JOptionPane.showMessageDialog(MemberGUI.this, "아이디는 8자이하!");
				tfId.requestFocus();
			} else {
			...
			String master = null;
			String birth = tfBirth.getText().trim();
			String regdate = tfRegdate.getText().trim();
			String id = tfId.getText().trim();
		...
		} else if (obj == buttons[2]) {// 탈퇴
			setText(DELETE);
			int affectedRow = dao.delete(tfId.getText().trim());
			if (affectedRow == 1) {
				JOptionPane.showMessageDialog(buttons[2], "탈퇴 성공했어요!");
			} else
				JOptionPane.showMessageDialog(buttons[2], "해당아이디에 예약정보있음 등의 이유로\n탈퇴를 실패했습니다!");
			// 탈퇴처리후 다시 -1로 초기화
			selectedRow = -1;
			// 전체 데이타 뿌려주기
			setTableData(dao.getRecordAll());
			...
			if (fieldName.trim().equals("검색조건")) {
				...
				if (tfSearch.getText().trim().equals("")) {
					...
					if (fieldName.trim().equals("가입날짜")) {
						...
						List<MemberDTO> list = dao.search2(tfSearch.getText(),word2);
						if(list.isEmpty()){
						...
						List<MemberDTO> list = dao.search(fieldName, tfSearch.getText());
						if(list.isEmpty()){
							JOptionPane.showMessageDialog(this, "검색결과가 없습니다\n검색어를 확인해주세요");
						}else{
							setTableData(list);
			...
			if (fieldName.trim().equals("가입날짜")) {
				...
				tfSearch.setText("연락처 검색 예) 010-1234-5678 or 5678");
			} else if (fieldName.trim().equals("Email")) {
				tfSearch.setText("Email 검색 예) 1234@1234.xxx or @1234.xxx");
