...
public class ReservationGUI extends JPanel implements ActionListener, MouseListener,KeyListener,FocusListener  {
	...
	Object[] columnName = { "예약번호", "회원ID", "상품번호", "상품명", "상품수량", "예약일" };
	JTable table;
	...
	public void setTableData(List<ReservationDTO> list) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == btnadd) {
			...
			if (tf_id.getText().trim().equals("")) {
				...
				if (dao.reservInsert(tf_id.getText(), Integer.parseInt((tf_gid.getText()))) > 0) {
					JOptionPane.showMessageDialog(this, "추가되었습니다!");
					setTableData(dao.reservAll());
				} else
					JOptionPane.showMessageDialog(this, "추가를 실패했습니다!");
			}
			// 없는 아이디거나 없는 상품번호일때!!
		} else if (obj == btnupdate) {
			setText(UPDATE);
			List<ReservationDTO> list = dao.reservUpdate();
			setTableData(list);
			...
			if (fieldName.trim().equals("검색조건")) {
				...
				if (jtf.getText().trim().equals("")) {
					...
					List<ReservationDTO> list = dao.reservSearch2("gid",jtf.getText());
					if (list.isEmpty()) {
					...
					List<ReservationDTO> list = dao.reservSearch("packname", jtf.getText());
					if (list.isEmpty()) {
					...
					List<ReservationDTO> list = dao.reservSearch("id", jtf.getText());
					if (list.isEmpty()) {
					...
					List<ReservationDTO> list = dao.reservSearch3(jtf.getText(),word2);
					if (list.isEmpty()) {
